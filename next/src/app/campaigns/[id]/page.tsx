...
const CampaignDetail = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          if (
            ...
            campaignId !== apiCampaign.campaignSlug &&
            !campaignId?.includes("-")
          ) {
    ...
    const fallbackToContextData = () => {
      ...
      let foundCampaign = campaigns.find((c) => c._id === campaignId);

      // If not found by ID, try to find by slug
      if (!foundCampaign && campaignId) {
        foundCampaign = campaigns.find((c) => c.campaignSlug === campaignId);
      }
      ...
      if (foundCampaign) {
        ...
        if (
          ...
          campaignId !== foundCampaign.campaignSlug &&
          !campaignId?.includes("-")
        ) {
          window.history.replaceState(
            null,
  ...
  const daysLeft = getDaysLeft(campaign.deadline as number, campaign.endDate);
  ...
  const handleDonateClick = () => {
    router.push(
      `/campaigns/${campaign.campaignSlug || campaign._id || campaignId}/donate`
    );
  };
    ...
    <div className="min-h-screen flex flex-col">
      <div className="flex-grow">
        ...
        <div className="w-full h-64 md:h-96 bg-gray-200 relative">
          <Image
            src={`${process.env.NEXT_PUBLIC_BE_URL}${imageUrl}`}
            alt={campaign.title}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {campaign.media.additionalImages.map((img, idx) => (
                        ...
                        >
                          <Image
                            src={`${process.env.NEXT_PUBLIC_BE_URL}${img}`}
